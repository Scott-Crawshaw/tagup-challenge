...
# accepts JSON array of measurements and stores relevant information
@api.route('/data', methods = ["POST"])
def post_data():
    global measurements
    try:
        for data in request.json:
            assert "timestamp" in data and "value" in data and "sensor" in data
    ...
    # case where 'sensor', 'value', or 'timestamp' keys do not exist
    except AssertionError:
        return "All measurements must contain 'sensor', 'value', and 'timestamp' keys. Please check request and try again.", 400
